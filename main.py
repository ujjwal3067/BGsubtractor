...
# Get width and height of  // checking frane width and height
w = cap.get(3)
h = cap.get(4)
# Frame Area
frameArea = h * w
...
fgbg = cv2.createBackgroundSubtractorMOG2(
    detectShadows=True)  # we are also detecting shadows
...
while(cap.isOpened()):
    ...
    # reducing the RESOLUTION of the  image captured
    ...
    # Apply background subtractor again to get foreground mask2
    ...
    if ret == True:  # if code still getting input video frames
        ...
        for cnt in countours0:
            # finding the area bounded by given contour : cnt
            ...
            if area > areaTH:
                ...
                m = cv2.moments(cnt)
                # Centroid
                # x coordinate of the centroid of the detected vehicle
                cx = int(m['m10'] / m['m00'])
                # y coorddinate of the centroid of the detected vehicle
                cy = int(m['m01'] / m['m00'])
                ...
                x, y, w, h = cv2.boundingRect(
                    cnt)  # gives rectangular overlay of contour cnt
                ...
                if cy in range(up_limit, down_limit):
                    for i in cars:
                        ...
                        if abs(x - i.getX()) <= w and abs(y - i.getY()) <= h:
                            ...
                            # if car is going up the way
                            if i.going_UP(line_down, line_up) == True:
                                cnt_up += 1  # up going cars counter
                                #print("ID:",i.getId(),'crossed going up at', time.strftime("%c"))
                            # if car is going down the way
                        ...
                        if i.timedOut():  # if done = true
                            # index=cars.index(i)
                            # removing the car from the cars list
                ...
                # -1 for completely filling the centroid circle
                cv2.circle(frame, (cx, cy), 5, (231, 219, 0), -1)
        ...
        # string to be displayed on the output screen "UP : cnt_up"
        str_up = 'VEHICLE GOING UP : ' + str(cnt_up)
        # string to be displayed on the oupput screen "DOWN :  cnt_down"
        str_down = 'VEHICLE GOING DOWN: ' + str(cnt_down)
        '''
        ...
        cv2.putText(frame, str_up, (10, 40), font, 0.5,
                    (255, 255, 255), 2, cv2.LINE_AA)  # white color line
        cv2.putText(frame, str_up, (10, 40), font, 0.5,
                    (0, 0, 0), 1, cv2.LINE_AA)  # red color line
        cv2.putText(frame, str_down, (10, 90), font, 0.5,
                    (0, 0, 0), 2, cv2.LINE_AA)  # white color line
        cv2.putText(frame, str_down, (10, 90), font, 0.5,
                    (0, 0, 255), 1, cv2.LINE_AA)  # blue color line
